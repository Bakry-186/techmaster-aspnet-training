...
public interface IBookStoreService
{
    ...
    Task<(bool Success, string? ErrorCode)> DeleteAuthor(int id);

    Task<IReadOnlyList<Category>> GetCategories();
    Task<Category?> CreateCategory(CreateCategoryDto dto);

    Task<PagedBooksResponse> GetBooks(string? search, int? authorId, int? categoryId, bool? isAvailable, int pageNumber, int pageSize);
